...
describe('operating worlds', () => {
  ...
  it('maps public hubs and jobs to distinct worlds instead of one generic intake', () => {
    ...
    expect(worldFromPublicParams({ guide: 'moving-data-between-systems' })).toBe('business')
    expect(worldFromPublicParams({ source: 'project-moving-data-between-systems' })).toBe('business')
    expect(worldFromPublicParams({ guide: 'ongoing-administrative-support' })).toBe('business')
