...
export default function Signup() {
  ...
  useEffect(() => {
    ...
    api.get<InviteResponse>(`/invite/${encodeURIComponent(inviteToken)}`)
      .then((r) => {
        ...
        setForm((f) => ({ ...f, first_name: parts[0] || f.first_name, last_name: parts.slice(1).join(' ') || f.last_name, email: r.invite.email || f.email }))
  ...
  return (
    <AuthLayout
      eyebrow={inviteToken ? 'Pinnacle invitation' : 'Client portal'}
      title={inviteToken ? 'Set up your Pinnacle account' : 'Create your account'}
      subtitle={inviteToken ? 'Your private invitation connects this account to your existing conversation with Pinnacle and expires after 24 hours.' : 'Basic info to get started — you can explore services and add details once you’re in.'}
      footer={<>Already have an account? <Link to="../login" className="font-medium text-gold hover:underline">Sign in</Link></>}
    >
      {inviteLoading && <p className="mb-4 text-sm text-slate-400">Loading invitation…</p>}
      {inviteToken && invite?.status === 'pending' && <div className="mb-5 rounded-xl border border-gold/20 bg-gold/[0.05] p-4"><p className="text-sm font-semibold text-white">Invited by Pinnacle</p><p className="mt-1 text-xs text-slate-400">This private link expires {new Date(invite.expires_at).toLocaleString()}.</p></div>}
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={busy || inviteLoading || (!!inviteToken && invite?.status !== 'pending')} className="btn-gold w-full disabled:opacity-60">{busy ? 'Creating account…' : 'Create account'}</button>
      </form>
    </AuthLayout>
  )
}
