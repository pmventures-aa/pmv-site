...
export default function VendorSignup() {
  ...
  const applicationData = {
    ...
    commission_state:form.commission_state || null,
  ...
  return (
    <AuthLayout surface="staff" eyebrow={inviteToken ? 'Private Pinnacle provider invitation' : trackCopy.eyebrow} title={stepTitle} subtitle={stepSubtitle} sideLabel={trackCopy.badge} sideTitle={trackCopy.sideTitle} sideBody={trackCopy.sideBody} footer={<>Already approved? <Link to="../login" className="font-medium text-gold hover:underline">Sign in</Link></>}>
      ...
      {step === 'questions' && <div className="space-y-5">
        ...
        <div className="rounded-xl border border-white/[.08] bg-white/[.02] p-4"><p className="mb-1 text-sm font-semibold text-white">Notary commission</p><p className="mb-4 text-xs leading-5 text-slate-500">{notarySelected ? 'Required for notary and signing work.' : 'If you hold an active notary commission, enter its number so we have it on file. Optional if you do not.'}</p><div className="grid gap-3 sm:grid-cols-2"><Field label={notarySelected ? 'Commission number' : 'Notary commission number (optional)'}><input className={inputCls} value={form.commission_number} onChange={(e)=>set('commission_number',e.target.value)}/></Field><Field label={notarySelected ? 'Commission state' : 'Commission state (optional)'}><input className={inputCls} value={form.commission_state} onChange={(e)=>set('commission_state',e.target.value)}/></Field>{notarySelected && <Field label="Commission expiration"><input className={inputCls} type="date" value={form.commission_expiration} onChange={(e)=>set('commission_expiration',e.target.value)}/></Field>}{ronSelected && <Field label="RON platform / provider"><input className={inputCls} placeholder="Example: Proof, Notarize, BlueNotary" value={form.ron_provider} onChange={(e)=>set('ron_provider',e.target.value)}/></Field>}</div></div>
