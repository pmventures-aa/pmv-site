...
export default function InvitationsAdmin() {
  ...
  // Live preview of the exact email the recipient will get, from the server's
  // own inviteCopy() (no drift), so the personal note is seen in context.
  const [preview, setPreview] = useState<{ firstName:string; subject:string; title:string; body:string; cta:string } | null>(null)
  useEffect(() => {
    if (!showForm) return
    const t = setTimeout(() => {
      api.post<{ firstName:string; subject:string; title:string; body:string; cta:string }>('/admin/invitations/preview', {
        invite_type: form.invite_type, full_name: form.full_name, company_name: form.company_name,
        vendor_category: form.vendor_category, known_services: form.known_services, personal_note: form.personal_note,
      }).then(setPreview).catch(() => {})
    }, 400)
    return () => clearTimeout(t)
  }, [showForm, form.invite_type, form.full_name, form.company_name, form.vendor_category, form.known_services, form.personal_note])
