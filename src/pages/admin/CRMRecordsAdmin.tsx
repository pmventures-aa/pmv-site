...
import { campaignAudienceHref } from '../../lib/engagements'
...
function parseCsv(text: string): Record<string, string>[] {
  ...
  const aliases: Record<string, string> = {
    ...
    contact: 'contact_name', contact_person: 'contact_name', contact_name: 'contact_name',
    contact_first: 'contact_first_name', contact_first_name: 'contact_first_name', contact_last: 'contact_last_name', contact_last_name: 'contact_last_name',
  }
    ...
    <div>
        ...
        <Panel className="mb-6">
          <form onSubmit={createRecord}>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {draft.record_type === 'business' && <input className={inputCls} placeholder="Business name" required value={draft.company_name} onChange={(e) => setDraft((d) => ({ ...d, company_name: e.target.value }))} />}
              <input className={inputCls} placeholder={draft.record_type === 'business' ? 'Contact first name' : 'First name'} required={draft.record_type === 'person'} value={draft.first_name} onChange={(e) => setDraft((d) => ({ ...d, first_name: e.target.value }))} />
              <input className={inputCls} placeholder={draft.record_type === 'business' ? 'Contact last name' : 'Last name'} value={draft.last_name} onChange={(e) => setDraft((d) => ({ ...d, last_name: e.target.value }))} />
              {draft.record_type === 'person' && <input className={inputCls} placeholder="Company" value={draft.company_name} onChange={(e) => setDraft((d) => ({ ...d, company_name: e.target.value }))} />}
              <input className={inputCls} placeholder={draft.record_type === 'business' ? 'Contact title' : 'Job title'} value={draft.job_title} onChange={(e) => setDraft((d) => ({ ...d, job_title: e.target.value }))} />
      ...
      {tab === 'records' && <>
        <div className="mb-4 grid gap-3 md:grid-cols-[minmax(220px,1fr)_160px_170px_150px]">
          <input className={inputCls} placeholder="Search name, contact, email, company or phone…" value={query} onChange={(e) => setQuery(e.target.value)} />
          ...
          <select className={inputCls} value={lifecycle} onChange={(e) => setLifecycle(e.target.value)}><option value="">All lifecycle stages</option><option value="lead">Lead</option><option value="prospect">Prospect</option><option value="opportunity">Application</option><option value="lost">Lost</option></select>
