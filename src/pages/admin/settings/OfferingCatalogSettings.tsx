...
export function OfferingCatalogSettings({serviceKey}:{serviceKey:string}){
  ...
  return <div className="border-t border-white/10 px-5 py-4"><div className="mb-3 flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-white">One-time & project offerings</p><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">Retail pricing and client scope are public. Provider payout guidance is internal to HQ. Hide an offering without deleting it when it is temporarily unavailable.</p></div><button type="button" className={`${btnOutline} text-xs`} onClick={()=>setShowNew(v=>!v)}>{showNew?'Cancel':'+ Add offering'}</button></div>
    {showNew&&<form onSubmit={create} className="mb-4 grid gap-3 rounded-xl border border-gold/20 bg-gold/[.035] p-4 sm:grid-cols-2">
      <label><span className="mb-1 block text-[11px] text-slate-500">Offering ID</span><input required className={inputCls} placeholder="field-new-service" value={form.id} onChange={e=>setForm(f=>({...f,id:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Name</span><input required className={inputCls} value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></label>
      <label className="sm:col-span-2"><span className="mb-1 block text-[11px] text-slate-500">Description</span><textarea required className={`${inputCls} min-h-[70px]`} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Starting retail price</span><input className={inputCls} type="number" min="0" step="0.01" value={form.startingPrice} onChange={e=>setForm(f=>({...f,startingPrice:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Pricing label</span><input className={inputCls} placeholder="Starting at $99" value={form.pricingLabel} onChange={e=>setForm(f=>({...f,pricingLabel:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Badge</span><input className={inputCls} placeholder="Licensed professional" value={form.badge} onChange={e=>setForm(f=>({...f,badge:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Sort order</span><input className={inputCls} type="number" value={form.sortOrder} onChange={e=>setForm(f=>({...f,sortOrder:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Starting information shown to client</span><textarea className={`${inputCls} min-h-[90px]`} placeholder="Property address\nPurpose / use case" value={form.requirements} onChange={e=>setForm(f=>({...f,requirements:e.target.value}))}/></label>
      <label><span className="mb-1 block text-[11px] text-slate-500">Required documents</span><textarea className={`${inputCls} min-h-[90px]`} placeholder="Government-issued ID\nProof of insurance" value={form.requiredDocuments} onChange={e=>setForm(f=>({...f,requiredDocuments:e.target.value}))}/></label>
      <label className="sm:col-span-2"><span className="mb-1 block text-[11px] text-slate-500">Offering-specific required intake prompts</span><textarea className={`${inputCls} min-h-[90px]`} placeholder="What is the property address?\nWhat is the intended use of the report?" value={form.intakePrompts} onChange={e=>setForm(f=>({...f,intakePrompts:e.target.value}))}/></label>
      <label className="sm:col-span-2"><span className="mb-1 block text-[11px] text-slate-500">Client scope / disclaimer note</span><textarea className={`${inputCls} min-h-[70px]`} value={form.note} onChange={e=>setForm(f=>({...f,note:e.target.value}))}/></label>
      <div className="sm:col-span-2 rounded-lg border border-gold/15 bg-gold/[.025] p-3"><p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-gold">HQ-only provider economics</p><div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1 block text-[11px] text-slate-500">Provider payout min</span><input className={inputCls} type="number" min="0" step="0.01" value={form.providerPayoutMin} onChange={e=>setForm(f=>({...f,providerPayoutMin:e.target.value}))}/></label><label><span className="mb-1 block text-[11px] text-slate-500">Provider payout max</span><input className={inputCls} type="number" min="0" step="0.01" value={form.providerPayoutMax} onChange={e=>setForm(f=>({...f,providerPayoutMax:e.target.value}))}/></label><label className="sm:col-span-2"><span className="mb-1 block text-[11px] text-slate-500">Provider payout / dispatch notes</span><textarea className={`${inputCls} min-h-[70px]`} value={form.providerPayoutNotes} onChange={e=>setForm(f=>({...f,providerPayoutNotes:e.target.value}))}/></label></div></div>
      <div className="sm:col-span-2"><button disabled={busy} className={btnPrimary}>{busy?'Creating…':'Create offering'}</button></div>
    </form>}
