        ...
        <Panel className="mb-5 overflow-x-auto !p-0">
          ...
          <table className="w-full min-w-[520px] text-sm">
            ...
            <tbody>{users.map((u) => <tr key={u.id} className="border-t border-white/5">
              <td className="px-5 py-2 text-slate-200">{u.full_name || u.email}</td><td className="px-5 py-2"><Tag tone={u.role === 'admin' ? 'gold' : 'slate'}>{u.role}</Tag></td><td className="px-5 py-2">{u.is_owner ? '✓' : '—'}</td><td className="px-5 py-2">{u.role === 'admin' || u.can_reveal_payment_info ? '✓' : '—'}</td><td className="px-5 py-2">{u.role === 'admin' || u.can_manage_users ? '✓' : '—'}</td><td className="px-5 py-2">{u.role === 'admin' || u.can_manage_settings ? '✓' : '—'}</td><td className="px-5 py-2">{u.role === 'admin' || u.can_view_reports ? '✓' : '—'}</td><td className="px-5 py-2">{u.role === 'admin' || u.can_view_audit_log ? '✓' : '—'}</td>
            </tr>)}</tbody>
          </table>
        </Panel>
      )}
