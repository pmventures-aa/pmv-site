  ...
  return <div>
    ...
    <Panel className="mb-6">
      ...
      <form onSubmit={assign} className="mt-4 flex flex-col gap-3 md:flex-row md:items-end">
        <label className="flex-1"><span className="mb-1 block text-xs text-slate-400">Team member</span><select className={inputCls} required value={assignUser} onChange={(e)=>setAssignUser(e.target.value)}><option value="">Choose employee or vendor…</option>{employees.filter((e)=>e.status!=='suspended').map((e)=><option key={e.id} value={e.id}>{e.full_name||e.email} · {e.party_type==='vendor'?'Vendor':'Employee'}</option>)}</select></label>
