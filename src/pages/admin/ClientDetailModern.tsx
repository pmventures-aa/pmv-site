...
function DetailRow({ label, value, children }: { label: string; value?: React.ReactNode; children?: React.ReactNode }) {
  return <div className="grid gap-1 border-t border-white/5 py-3 first:border-t-0 sm:grid-cols-[150px_minmax(0,1fr)] sm:gap-5"><dt className="text-xs font-medium uppercase tracking-wide text-slate-500">{label}</dt><dd className="min-w-0 text-sm text-slate-200">{children ?? value ?? '—'}</dd></div>
}
    ...
    {tab === 'overview' && <div className="mt-7 grid gap-9 xl:grid-cols-[minmax(0,1fr)_380px]">
      <main>
        ...
        {editing ? <ProfileEditor account={account} profile={profile} onSaved={async () => { setEditing(false); await load() }} /> : <dl className="border-y border-white/10">
          <DetailRow label="Business" value={profile?.business_name || '—'} />
          <DetailRow label="Entity" value={profile?.entity_type || '—'} />
        ...
        <div className="mt-9">
          ...
          {!data.services.length ? <EmptyState label="No services are attached to this client." /> : <div className="border-y border-white/10">{data.services.slice(0, 8).map((s) => <div key={s.id} className="flex items-center justify-between gap-4 border-t border-white/5 py-3.5 first:border-t-0"><div><p className="text-sm font-medium text-white">{s.name}</p><p className="mt-0.5 text-xs text-slate-500">{s.service_key.replace(/_/g, ' ')}</p></div><Tag tone={statusTone(s.status)}>{s.status.replace(/_/g, ' ')}</Tag></div>)}</div>}
        </div>
