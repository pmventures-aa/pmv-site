...
function SimpleTable({ columns, rows, empty }: { columns: { key: string; label: string; render?: (row: any) => React.ReactNode }[]; rows: any[]; empty: string }) {
  ...
  return <><ul className="space-y-2.5 py-3 md:hidden">{rows.map((row, idx) => <li key={row.id ?? idx} className="space-y-1.5 rounded-lg border border-white/10 bg-white/[.02] p-3">{columns.map((c) => <div key={c.key} className="flex justify-between gap-3 text-xs"><span className="shrink-0 uppercase tracking-wide text-slate-500">{c.label}</span><span className="min-w-0 text-right text-slate-300">{c.render ? c.render(row) : String(row[c.key] ?? 'Not provided')}</span></div>)}</li>)}</ul><div className="hidden overflow-x-auto border-y border-white/10 md:block"><table className="w-full min-w-[640px] text-left text-sm"><thead><tr className="text-[11px] uppercase tracking-wide text-slate-500">{columns.map((c) => <th key={c.key} className="px-3 py-2.5 font-medium">{c.label}</th>)}</tr></thead><tbody>{rows.map((row, idx) => <tr key={row.id ?? idx} className="border-t border-white/5"><>{columns.map((c) => <td key={c.key} className="px-3 py-3 text-slate-300">{c.render ? c.render(row) : String(row[c.key] ?? 'Not provided')}</td>)}</></tr>)}</tbody></table></div></>
}
