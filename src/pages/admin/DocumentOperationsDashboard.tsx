...
export default function DocumentOperationsDashboard(){
  ...
  return <div>
    <Panel className="mb-5 !p-0 overflow-hidden border-gold/15 bg-gradient-to-br from-gold/[.055] via-white/[.018] to-transparent">
      <div className="border-b border-white/10 px-5 py-5 sm:px-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-gold/80">Document Control</p><h2 className="mt-2 text-xl font-extrabold tracking-tight text-white">Enterprise document operations</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Manage working documents, controlled sharing, version history, signature transactions, evidence, and governance from one operational system.</p></div><div className="flex flex-wrap gap-2"><Link to={p('envelopes')} className={btnSecondary}><Send size={14}/>Envelope Workspace</Link><Link to={p('community-documents')} className={btnSecondary}><BookOpen size={14}/>Template Library</Link></div></div></div>
    ...
    <div className="mb-5 grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      ...
      <Panel><p className="text-xs font-extrabold text-white">Governance shortcuts</p><p className="mt-1 text-xs leading-5 text-slate-500">Sensitive document operations should always have an evidence and access path.</p><div className="mt-4 space-y-2"><Link to={p('audit-log')} className="flex items-center justify-between rounded-xl border border-white/10 px-3.5 py-3 text-sm font-semibold text-slate-300 hover:border-gold/25 hover:text-gold"><span className="flex items-center gap-2"><ShieldCheck size={15}/>Document & access audit</span><Tag>Audit</Tag></Link><Link to={p('security-center')} className="flex items-center justify-between rounded-xl border border-white/10 px-3.5 py-3 text-sm font-semibold text-slate-300 hover:border-gold/25 hover:text-gold"><span className="flex items-center gap-2"><LockKeyhole size={15}/>Security & sessions</span><Tag>Security</Tag></Link><Link to={p('envelopes')} className="flex items-center justify-between rounded-xl border border-white/10 px-3.5 py-3 text-sm font-semibold text-slate-300 hover:border-gold/25 hover:text-gold"><span className="flex items-center gap-2"><FileCheck2 size={15}/>Executed evidence packages</span><Tag>Evidence</Tag></Link></div></Panel>
    </div>
