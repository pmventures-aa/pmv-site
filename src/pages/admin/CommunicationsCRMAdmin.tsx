...
import { Search, Send, FileText, CalendarClock, AlertTriangle, Inbox, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react'
...
interface AudienceData {
  ...
  leads: { id: string; name: string; email: string; phone: string | null; company_name: string | null; first_name: string | null; last_name: string | null; job_title: string | null; record_type: string; lifecycle_stage: string; status: string; email_status: string }[]
...
export default function CommunicationsCRMAdmin() {
  ...
  const recordMatches = useMemo(() => {
    ...
    return { people: audience.people.filter((x) => !q || `${x.full_name || ''} ${x.email}`.toLowerCase().includes(q)), leads: audience.leads.filter((x) => !q || `${x.name} ${x.email} ${x.company_name || ''} ${x.first_name || ''} ${x.last_name || ''} ${x.job_title || ''}`.toLowerCase().includes(q)) }
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <div className="mb-3 flex snap-x gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden xl:hidden">
      {folderItems.map((item) => { const Icon = item.icon; return <button key={item.key} onClick={() => { setFolder(item.key); setSelectedId(null) }} className={`inline-flex shrink-0 snap-start items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold ${folder === item.key ? 'border-gold/50 bg-gold/10 text-gold' : 'border-white/10 text-slate-400'}`}><Icon size={13}/>{item.label}<span className="text-[10px] opacity-70">{item.count}</span></button> })}
    </div>

    <Panel className="flex min-h-[620px] flex-col overflow-hidden !p-0 xl:grid xl:min-h-[690px] xl:grid-cols-[220px_360px_minmax(0,1fr)]">
      <aside className="hidden border-b border-white/10 p-3 xl:block xl:border-b-0 xl:border-r">
      ...
      <section className={`${selectedId ? 'hidden' : 'flex'} min-h-0 flex-1 flex-col border-b border-white/10 xl:flex xl:border-b-0 xl:border-r`}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto xl:max-h-[620px]">
      ...
      <section className={`${selectedId ? 'flex' : 'hidden'} min-h-0 min-w-0 flex-1 flex-col xl:flex`}>
        {!selectedId ? <div className="grid h-full min-h-[500px] place-items-center p-8"><div className="max-w-md text-center"><Inbox className="mx-auto mb-4 text-slate-600" size={34} /><h2 className="text-lg font-semibold text-white">Select a message</h2><p className="mt-2 text-sm leading-6 text-slate-500">Open a message to review content, recipients, delivery status, scheduling, and any exceptions.</p></div></div> : detailLoading ? <p className="p-6 text-sm text-slate-400">Loading message…</p> : detail ? <><div className="flex min-h-0 flex-1 flex-col"><div className="sticky top-0 z-10 flex items-center gap-2 border-b border-white/10 bg-navy-950/90 px-3 py-2 backdrop-blur xl:hidden"><button type="button" onClick={() => setSelectedId(null)} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/15 text-slate-300" aria-label="Back to email list"><ChevronLeft size={17}/></button><p className="min-w-0 flex-1 truncate text-sm font-semibold text-white">{detail.message.subject}</p><Tag tone={statusTone[detail.message.status] || 'slate'}>{detail.message.status}</Tag></div><div className="min-h-0 flex-1 overflow-y-auto p-4 pb-24 sm:p-6 xl:pb-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[.16em] text-slate-500">{detail.message.message_type} email</p><h2 className="mt-1 text-xl font-semibold text-white">{detail.message.subject}</h2><p className="mt-2 text-sm text-slate-500">From {detail.message.created_by_name || detail.message.created_by_email || 'Pinnacle Management Ventures'} · {fmt(detail.message.created_at)}</p>{detail.message.scheduled_at && <p className="mt-1 text-xs text-slate-500">Scheduled {fmt(detail.message.scheduled_at)}{detail.message.recurrence ? ` · ${detail.message.recurrence}` : ''}{detail.message.next_run_at ? ` · next ${fmt(detail.message.next_run_at)}` : ''}</p>}</div><Tag tone={statusTone[detail.message.status] || 'slate'}>{detail.message.status}</Tag></div>
          ...
          <div className="mt-6 hidden flex-wrap gap-2 xl:flex">{caps.can_manage_communications && detail.message.status === 'draft' && <button className={btnPrimary} onClick={() => sendDraft(detail.message.id)}>Send now</button>}{caps.can_manage_communications && ['draft','queued'].includes(detail.message.status) && <button className={btnOutline} onClick={() => cancelMessage(detail.message.id)}>Cancel</button>}</div>
          <div className="mt-8"><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold text-white">Recipient delivery</h3><span className="text-xs text-slate-500">Provider-backed status</span></div>{detail.recipients.length === 0 ? <EmptyState label="Recipients will be resolved when this draft is sent." /> : <><div className="divide-y divide-white/10 rounded-md border border-white/10 xl:hidden">{detail.recipients.map((r) => <article key={r.id} className="p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-200">{r.full_name || r.email}</p><p className="truncate text-xs text-slate-500">{r.email}</p></div><Tag tone={statusTone[r.status] || 'slate'}>{r.status}</Tag></div><dl className="mt-3 grid gap-2 text-xs"><div><dt className="text-slate-600">Sent</dt><dd className="text-slate-400">{fmt(r.sent_at)}</dd></div><div><dt className="text-slate-600">Details</dt><dd className="text-slate-400">{r.error || (r.provider_message_id ? 'Provider delivery tracking active.' : 'Awaiting delivery.')}</dd></div></dl></article>)}</div><div className="hidden overflow-x-auto rounded-md border border-white/10 xl:block"><table className="w-full min-w-[650px] text-sm"><thead><tr className="border-b border-white/10 text-left text-xs uppercase tracking-wide text-slate-500"><th className="px-4 py-3">Recipient</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Sent</th><th className="px-4 py-3">Details</th></tr></thead><tbody>{detail.recipients.map((r) => <tr key={r.id} className="border-b border-white/5 last:border-0"><td className="px-4 py-3"><p className="text-slate-200">{r.full_name || r.email}</p><p className="text-xs text-slate-500">{r.email}</p></td><td className="px-4 py-3"><Tag tone={statusTone[r.status] || 'slate'}>{r.status}</Tag></td><td className="px-4 py-3 text-slate-400">{fmt(r.sent_at)}</td><td className="max-w-[260px] px-4 py-3 text-xs text-slate-500">{r.error || (r.provider_message_id ? 'Provider delivery tracking active.' : 'Awaiting delivery.')}</td></tr>)}</tbody></table></div></>}</div>
        </div><div className="sticky bottom-0 z-10 flex gap-2 border-t border-white/10 bg-navy-950/90 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur xl:hidden">{caps.can_manage_communications && detail.message.status === 'draft' && <button className={`${btnPrimary} flex-1`} onClick={() => sendDraft(detail.message.id)}>Send now</button>}{caps.can_manage_communications && ['draft','queued'].includes(detail.message.status) && <button className={btnOutline} onClick={() => cancelMessage(detail.message.id)}>Cancel</button>}</div></div></> : <div className="p-6"><EmptyState label="This message could not be loaded." /></div>}
      </section>
    </Panel>

    <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
      <DialogContent size="xl" className="max-h-[calc(100dvh-1rem)] overflow-hidden !p-4 sm:max-h-none sm:overflow-visible sm:!p-6" title="New email" description="Compose a professional message and select the exact audience, delivery time, and communication type.">
        {!caps.can_manage_communications ? <NoAccess label="sending communications" /> : <div className="max-h-[calc(100dvh-8rem)] overflow-x-hidden overflow-y-auto pr-0 sm:max-h-[78vh] sm:pr-1"><div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section className="min-w-0"><div className="mb-4 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-sm font-semibold text-white">Message</p><p className="mt-1 text-xs text-slate-500">Write the final email exactly as the recipient should receive it.</p></div><select className={`${inputCls} w-full sm:!w-auto`} value={messageType} onChange={(e) => setMessageType(e.target.value as any)}><option value="operational">Client / operational</option><option value="internal">Internal team</option><option value="marketing">Marketing / outreach</option></select></div><div className="min-w-0 space-y-4"><input className={inputCls} placeholder="Subject" value={subject} onChange={(e) => setSubject(e.target.value)} /><RichTextComposer value={bodyHtml} onChange={setBodyHtml} onUploadImage={uploadImage} /><div className="flex flex-wrap items-center gap-3"><label className="flex w-full min-w-0 flex-col items-stretch gap-2 text-xs font-semibold uppercase tracking-[.12em] text-slate-400 sm:w-auto sm:flex-row sm:items-center">Signature<select className={`${inputCls} w-full min-w-0 normal-case tracking-normal sm:!w-auto sm:min-w-[180px]`} value={signatureId} onChange={(e) => setCampaignSignature(e.target.value)}><option value="none">None</option>{signatures.map((s) => <option key={s.id} value={s.id}>{signatureLabel(s)}</option>)}</select></label></div>{selectedCampaignSignature && <div className="max-w-full overflow-hidden rounded-md border border-white/10 bg-white p-3 sm:p-4"><SignaturePreview signature={selectedCampaignSignature} /></div>}</div></section>
        ...
        </div><div className="sticky bottom-0 z-10 mt-5 flex flex-wrap justify-end gap-2 border-t border-white/10 bg-navy-900/95 pb-[max(0.25rem,env(safe-area-inset-bottom))] pt-4 backdrop-blur xl:static xl:bg-transparent xl:pb-0 xl:backdrop-blur-none"><button disabled={busy} className={btnOutline} onClick={() => submit('draft')}>Save draft</button><button disabled={busy} className={btnPrimary} onClick={() => submit(sendMode==='schedule' ? 'schedule' : 'send_now')}>{busy ? 'Working…' : sendMode==='schedule' ? 'Schedule email' : 'Send email'}</button></div></div>}
      </DialogContent>
    </Dialog>
  </div>
}
