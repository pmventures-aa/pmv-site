    ...
    <div className="space-y-6">
      ...
      <Panel className="overflow-x-auto p-0">
        {loading ? (
          ...
          <>
          <ul className="space-y-2.5 p-3 md:hidden">{jobs.map((job) => <li key={job.id} className="rounded-lg border border-white/10 bg-white/[.02] p-4"><button onClick={() => setSelectedId(job.id)} className="block text-left"><p className="font-medium text-white hover:text-gold">{job.serviceLabel}</p><p className="text-xs text-slate-400">{job.reference} · {job.tierLabel || `${job.bedrooms}BR`}/{job.bathrooms}BA · {COUNTY_LABEL[job.county] || job.county}</p></button><div className="mt-2 flex flex-wrap items-center gap-1.5"><span className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${toneClass[cleaningStatusTone(job.status)]}`}>{cleaningStatusLabel(job.status)}</span>{job.openIssues > 0 && <span className="inline-block rounded bg-rose-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-rose-300">{job.openIssues} issue{job.openIssues > 1 ? 's' : ''}</span>}{job.turnover.urgency !== 'none' && job.turnover.minutesRemaining != null && <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${job.turnover.urgency === 'late' || job.turnover.urgency === 'at_risk' ? 'bg-rose-500/15 text-rose-300' : 'bg-amber-500/15 text-amber-300'}`}>{job.turnover.minutesRemaining < 0 ? 'Late' : `${Math.floor(job.turnover.minutesRemaining / 60)}h ${job.turnover.minutesRemaining % 60}m left`}</span>}</div><dl className="mt-3 space-y-1.5 text-xs"><div className="flex justify-between gap-3"><dt className="text-slate-500">Schedule</dt><dd className="text-right text-slate-300">{job.scheduledDate || <span className="text-amber-300">Unscheduled</span>}{job.arrivalWindow && <span className="block text-slate-500">{job.arrivalWindow}</span>}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Client</dt><dd className="tabular-nums text-slate-200">{formatUsd(job.clientTotalCents)}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Payout</dt><dd className="tabular-nums text-slate-300">{formatUsd(job.vendorPayoutCents)}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Margin</dt><dd className={`tabular-nums ${job.belowMinMargin ? 'text-rose-300' : 'text-emerald-300'}`}>{formatUsd(job.marginCents)} <span className="text-[10px] text-slate-500">{job.marginPercent}%{job.belowMinMargin ? ' · low' : ''}</span></dd></div><div className="flex items-center justify-between gap-3"><dt className="shrink-0 text-slate-500">Vendor</dt><dd className="text-right">{job.assignedVendorName ? <span className="text-slate-200">{job.assignedVendorName}</span> : <select className={`${inputCls} max-w-[150px] py-1 text-xs`} defaultValue="" onChange={(e) => assign(job, e.target.value)}><option value="" disabled>Assign…</option>{vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</select>}</dd></div></dl><div className="mt-3 flex flex-wrap gap-3 border-t border-white/5 pt-3">{job.status === 'needs_review' && <button onClick={() => setStatus(job, 'scheduled')} className="text-xs font-semibold text-gold hover:underline">Approve</button>}{job.status === 'requested' && <button onClick={() => setStatus(job, 'scheduled')} className="text-xs font-semibold text-gold hover:underline">Schedule</button>}{!['completed', 'cancelled'].includes(job.status) && <button onClick={() => setStatus(job, 'cancelled')} className="text-xs text-slate-500 hover:text-rose-300">Cancel</button>}</div></li>)}</ul>
      ...
      {plans.length > 0 && (
        <div>
          ...
          <Panel className="overflow-x-auto p-0">
            <ul className="space-y-2.5 p-3 md:hidden">{plans.map((plan) => <li key={plan.id} className="rounded-lg border border-white/10 bg-white/[.02] p-4"><p className="font-medium text-white">{plan.serviceLabel}</p><p className="text-xs text-slate-400">{plan.reference} · {plan.contactName || plan.propertyAddress || COUNTY_LABEL[plan.county]}</p><dl className="mt-3 space-y-1.5 text-xs"><div className="flex justify-between gap-3"><dt className="text-slate-500">Cadence</dt><dd className="capitalize text-slate-300">{plan.frequency}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Next visit</dt><dd className="text-slate-300">{plan.status === 'active' ? (plan.nextDate || '-') : '-'}{plan.skipNext && plan.status === 'active' ? <span className="ml-1 text-amber-300">(skipping)</span> : null}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Visits</dt><dd className="text-slate-400">{plan.visitsCompleted}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Status</dt><dd><span className={`inline-block rounded px-2 py-0.5 text-[11px] font-semibold ${plan.status === 'active' ? 'bg-emerald-500/15 text-emerald-300' : plan.status === 'paused' ? 'bg-amber-500/15 text-amber-300' : 'bg-white/8 text-slate-400'}`}>{plan.status}</span></dd></div></dl><div className="mt-3 flex flex-wrap gap-3 border-t border-white/5 pt-3 text-xs">{plan.status === 'active' && <button onClick={() => planAction(plan, plan.skipNext ? 'unskip' : 'skip')} className="text-gold hover:underline">{plan.skipNext ? 'Unskip' : 'Skip next'}</button>}{plan.status === 'active' && <button onClick={() => planAction(plan, 'pause')} className="text-slate-400 hover:text-white">Pause</button>}{plan.status === 'paused' && <button onClick={() => planAction(plan, 'resume')} className="text-gold hover:underline">Resume</button>}{plan.status !== 'cancelled' && <button onClick={() => planAction(plan, 'cancel')} className="text-slate-500 hover:text-rose-300">Cancel</button>}</div></li>)}</ul>
