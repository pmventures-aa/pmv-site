  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {KPI_KEYS.map((key) => { const r=results[key]; const col=r?.columns[0]; const val=r?.rows[0]?.[col?.key??'']; if(loadingKeys.has(key)&&!r)return <SkeletonStatCard key={key}/>; return <Panel key={key} className="p-5"><p className="text-xs font-medium uppercase tracking-wide text-slate-400">{r?.label??key.replace(/_/g,' ')}</p><p className="mt-2 font-display text-3xl font-medium text-white tabular-nums">{loadingKeys.has(key)?'…':formatValue(val,col?.type)}</p></Panel> })}
      </div>
