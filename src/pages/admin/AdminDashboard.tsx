...
export default function AdminDashboard(){
  ...
  return <div className="pb-24 lg:pb-0">
    ...
    <div className="mt-7 grid gap-5 xl:grid-cols-[.9fr_1.1fr]">
      <Panel className="!p-0 overflow-hidden"><div className="flex items-center justify-between border-b border-white/[.07] px-5 py-4 sm:px-6"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">Schedule</p><h2 className="mt-1 text-sm font-semibold text-white">Upcoming appointments</h2></div></div>{!data?<p className="px-5 py-7 text-sm text-slate-400">Loading…</p>:data.upcoming_appointments.length===0?<div className="p-5"><EmptyState label="Nothing scheduled."/></div>:<ul className="divide-y divide-white/[.055]">{data.upcoming_appointments.map((a)=><li key={a.id} className="px-5 py-4 text-sm transition hover:bg-white/[0.018] sm:px-6"><Link to={p(`clients/${a.client_user_id}`)} className="font-medium text-white hover:text-gold">{a.title||'Appointment'}</Link><p className="mt-1.5 text-xs text-slate-500">{a.client_name||a.client_email} · {new Date(a.starts_at).toLocaleString()}</p></li>)}</ul>}</Panel>
      <Panel className="!p-0 overflow-hidden"><div className="flex items-center justify-between border-b border-white/[.07] px-5 py-4 sm:px-6"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">Firm activity</p><h2 className="mt-1 text-sm font-semibold text-white">Recent activity</h2></div><Link to={p('activity')} className="text-xs font-semibold text-gold hover:underline">View all</Link></div>{!data?<p className="px-5 py-7 text-sm text-slate-400">Loading…</p>:data.recent_activity.length===0?<div className="p-5"><EmptyState label="No activity yet."/></div>:<ul className="divide-y divide-white/[.055]">{data.recent_activity.slice(0,8).map((e)=><li key={e.id} className="px-5 py-4 text-sm transition hover:bg-white/[0.018] sm:px-6"><p className="text-slate-300">{describeActivity(e)}</p><p className="mt-1.5 text-xs text-slate-600">{timeAgo(e.created_at)}</p></li>)}</ul>}</Panel>
    </div>
