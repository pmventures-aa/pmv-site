...
export default function LeadDetail() {
  ...
  return (
    <div className="mx-auto max-w-[1440px]">
      ...
      {tab === 'overview' && <div className="mt-6 grid gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section>
          ...
          {editing ? <RecordEditor record={r} staff={staff} onSave={async (patch) => { await updateRecord(patch); setEditing(false) }} /> : <dl className="grid border-y border-white/10 sm:grid-cols-2">
            ...
            <Info label="Company / business" value={r.company_name || (r.record_type === 'business' ? r.name : '—')} />
            <Info label="Industry" value={r.industry || '—'} />
            <Info label="Source" value={r.source || '—'} />
      ...
      {tab === 'details' && <div className="mt-6 max-w-5xl"><div className="mb-5 flex items-end justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">Record data</p><h2 className="mt-1 text-xl font-semibold text-white">Contact & business details</h2></div><button onClick={() => setEditing((v) => !v)} className="text-sm font-medium text-gold hover:underline">{editing ? 'Cancel' : 'Edit'}</button></div>{editing ? <RecordEditor record={r} staff={staff} onSave={async (patch) => { await updateRecord(patch); setEditing(false) }} /> : <dl className="grid border-y border-white/10 sm:grid-cols-2"><Info label="Record type" value={r.record_type} /><Info label="Email status" value={r.email_status} /><Info label="Email" value={r.email} /><Info label="Phone" value={r.phone || '—'} /><Info label="Job title" value={r.job_title || '—'} /><Info label="Website" value={r.website || '—'} /><Info label="Industry" value={r.industry || '—'} /><Info label="Owner" value={r.owner_name || r.owner_email || 'Unassigned'} /><Info label="Address" value={[r.address_line1, r.address_line2, r.city, r.state, r.postal_code, r.country].filter(Boolean).join(', ') || '—'} /><Info label="Created" value={new Date(r.created_at).toLocaleString()} /></dl>}</div>}
    </div>
  )
}
