...
type SharedData={document:{title:string;description?:string|null;document_type:'text'|'file';text_content?:string|null};expires_at:string}

export default function SharedDocument(){
  const {token=''}=useParams(); const [data,setData]=useState<SharedData|null>(null); const [error,setError]=useState('')
  useEffect(()=>{fetch(`/api/shared/${encodeURIComponent(token)}`).then(async r=>{const d:any=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Unable to open document');return d as SharedData}).then(setData).catch(e=>setError(e instanceof Error?e.message:'Unable to open document'))},[token])
