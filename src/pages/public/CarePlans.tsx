...
export default function CarePlans(){
  ...
  const panelTransition = reduceMotion ? { duration: 0 } : pmvMotion.premium

  function chooseFamily(key: PlanFamilyKey) {
    setFamily(key)
...
function CarePlanIntake({family,initialTier}:{family:PlanFamilyKey;initialTier:string|null}){
  ...
  return <div className="overflow-hidden rounded-2xl border border-white/10 bg-navy-900/70 shadow-[0_28px_80px_rgba(0,0,0,.26)]">
    ...
    <div className="space-y-6 p-5 sm:p-6">
      ...
      <MagneticCta type="button" onClick={()=>void submit()} disabled={busy} className="w-full rounded-lg bg-gold px-5 py-3 text-sm font-bold text-navy-950 hover:bg-gold-300 disabled:opacity-60">{busy?'Sending intake…':`Send ${familyDef.eyebrow} intake`}</MagneticCta>
...
function InputField({label,children}:{label:string;children:ReactNode}){return <label className="flex flex-col gap-1.5 text-xs font-semibold text-slate-300">{label}{children}</label>}

function PropertyIntake({intake,update}:{intake:IntakeState;update:(k:string,v:IntakeState[string])=>void}){
  return <div className="border-t border-white/10 pt-5"><p className="text-sm font-bold text-white">About the property</p><div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2">
...
function BusinessIntake({intake,update}:{intake:IntakeState;update:(k:string,v:IntakeState[string])=>void}){
  return <div className="border-t border-white/10 pt-5"><p className="text-sm font-bold text-white">About the business</p><div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2">
...
function LegalIntake({intake,update}:{intake:IntakeState;update:(k:string,v:IntakeState[string])=>void}){
  return <div className="border-t border-white/10 pt-5"><p className="text-sm font-bold text-white">About your firm or practice</p><div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2">
