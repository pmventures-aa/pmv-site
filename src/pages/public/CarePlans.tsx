...
export default function CarePlans(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    ...
    <section className="sticky top-[56px] z-20 border-b border-white/10 bg-navy-900/95 backdrop-blur sm:top-[68px]"><div className="container-pmv -mx-4 flex gap-2 overflow-x-auto px-4 py-3 sm:mx-0 sm:flex-wrap sm:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">{CARE_PLAN_FAMILIES.map(key=>{const cfg=CARE_PLANS[key];const active=family===key;return <button key={key} type="button" onClick={()=>{setFamily(key);setSelectedTier(null)}} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition ${active?'border-gold/60 bg-gold/15 text-gold':'border-white/10 bg-white/[.02] text-slate-300 hover:border-white/25 hover:text-white'}`}>{cfg.eyebrow}</button>})}</div></section>
