...
const HUB_INTAKE: Record<'business'|'property'|'mobile', { to: string; cta: string; secondaryCta: string }> = {
  ...
  property: { to: '/scope-request?world=property&source=property-hub', cta: 'Request property work', secondaryCta: 'Start a property request' },
  mobile: { to: '/scope-request?world=documents&source=mobile-hub', cta: 'Request document or signing work', secondaryCta: 'Start a document request' },
}
...
const HUBS: Record<'business'|'property'|'mobile', HubConfig> = {
  ...
  property: {
    ...
    serviceSlugs: ['property-management','property-cleaning','eviction-support','property-inspections','field-photos-bpo'],
    themes: [
      ['Cleaning & turnovers','Standard cleans typically $150-250, deep cleans $275-500, and move-out turnovers $325-650 by size, with an instant online planning estimate.'],
      ['Inspections, photos & BPO','Occupancy checks from $75, exterior photo verification from $65, interior condition packages from $145, and licensed BPOs through our vetted network.'],
      ['Eviction & possession support','Notice preparation from $75, filing packets from $225, courthouse runs, docket tracking, lockout-day coordination, and post-possession turnover - all non-legal, with attorney handoff when needed.'],
      ['REO & property preservation','Securing, trash-out, vendor access, hazard follow-up, and ready-to-market coordination for vacant or bank-owned assets.'],
    ],
  ...
  mobile: {
    ...
    serviceSlugs: ['document-preparation','mobile-notary','remote-online-notary','document-courier'],
    themes: [
      ...
      ['Mobile notary','Coordinate eligible in-person notarizations at a convenient location - owned in South Florida, vetted signing agents elsewhere.'],
      ['Remote Online Notarization (RON)','Notarize documents from anywhere in the U.S. with a live licensed notary session and full audit record.'],
    ...
    metaTitle: 'Mobile Notary, RON & Document Services',
    metaDescription: 'Mobile notary, Remote Online Notarization (RON), document courier, signing support, and tracked document services from Pinnacle Management Ventures. Nationwide coverage.',
  },
}
...
function HubPage({ hub }: { hub: keyof typeof HUBS }) {
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    ...
    <section className="container-pmv py-14 sm:py-18"><Reveal className="grid gap-8 border-y border-gold/20 py-9 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="eyebrow">Ready to start this work?</p><h2 className="mt-2 font-display text-3xl font-medium text-white">This path stays in {cfg.eyebrow.toLowerCase()}.</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-400">You will not be sent through a generic catch-all form. The questions, portal, and follow-through match this operating world.</p></div><Link to={HUB_INTAKE[hub].to} className={btnPrimary}>{HUB_INTAKE[hub].secondaryCta}</Link></Reveal></section>
