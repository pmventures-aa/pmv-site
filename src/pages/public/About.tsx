...
const strengths:{icon:IconName;title:string;body:string}[]=[
  {icon:'team',title:'One relationship',body:'Bring the next need back to a team that already understands the context instead of rebuilding it from scratch.'},
  ...
  {icon:'activity',title:'Visible follow-through',body:'Projects, documents, messages, appointments, vendors, and next steps stay connected so fewer things disappear between people.'},
...
export default function About(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    ...
    <section className="container-pmv py-14 sm:py-16"><Reveal className="grid gap-10 lg:grid-cols-[.75fr_1.25fr]"><div><p className="eyebrow">How we work</p><h2 className="mt-3 font-display text-3xl font-medium text-white sm:text-4xl">Simple process. Clear responsibility.</h2><p className="mt-4 text-sm leading-6 text-slate-400">Good professional support is less about theatrics and more about context, communication, judgment, and follow-through.</p></div><div className="pmv-process-line">{[
      ...
      ['02','Define the work','Identify the scope, information, people, dependencies, and responsibilities.'],
    ...
    <CtaBand title="Have something that needs attention?" body="Start with the situation, not the service name. We can help determine the right scope and next step." primary={{to:'https://secure.pinnaclemanagementventures.com/signup',label:'Tell Us What You Need'}} secondary={{to:'/services',label:'View Services'}}/>
  </main><Footer/></div>
}
