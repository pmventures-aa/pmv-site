...
const personas:[string,string][]=[
  ['Landlords & property owners','/projects/property-cleaning-turnover?audience=landlord#proof'],
  ['Real estate agents & brokers','/projects/local-support-south-florida?audience=agent-broker#proof'],
  ['Real estate investors','/projects/eviction-turnover-support?audience=investor#proof'],
  ['Business owners & operators','/projects/ongoing-administrative-support?audience=business-owner#proof'],
  ['Attorneys & legal teams','/projects/eviction-turnover-support?audience=attorney#proof'],
  ['Multi-location operators','/projects/managing-business-transition?audience=operator#proof'],
]
...
export default function Home(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    ...
    <section className="border-b border-white/[.07] bg-navy-900/30"><div className="container-pmv py-8 sm:py-10"><Reveal><p className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-500">Built for</p><div className="mt-3 flex flex-wrap gap-2" aria-label="Find support by client type">{personas.map(([label,to])=><Link key={label} to={to} className="rounded-full border border-white/10 bg-white/[.03] px-4 py-2 text-xs font-semibold text-slate-300 transition hover:border-gold/50 hover:text-gold">{label} →</Link>)}</div></Reveal></div></section>
    ...
    <section className="container-pmv py-16 sm:py-24"><div className="grid gap-10 lg:grid-cols-[.58fr_1.42fr] lg:items-start lg:gap-14"><Reveal><p className="eyebrow">Start a request</p><h2 className="mt-4 font-display text-3xl font-bold tracking-[-.035em] text-white sm:text-4xl">One task or a full project - tell us what you need.</h2><p className="mt-5 max-w-md text-sm leading-7 text-slate-400">A cleaning, inspection, document, notary appointment, field visit, or larger multi-step project all start the same way: tell us what needs to happen, where, and when. No account and no formal brief required.</p><p className="mt-4 flex items-center gap-2 text-sm font-semibold text-white"><span className="inline-block h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(74,222,128,.6)]"/>A real person replies within two business hours.</p></Reveal><ScopeWizard source="home" compact/></div></section>
