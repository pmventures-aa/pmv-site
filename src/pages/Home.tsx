...
const personas:{label:string;benefit:string;to:string}[]=[
  {label:'Landlords & Property Owners',benefit:'Keep turnovers, inspections, and vendors moving',to:'/projects/property-cleaning-turnover?audience=landlord#proof'},
  {label:'Real Estate Agents & Brokers',benefit:'On-site help so listings and closings stay on time',to:'/projects/local-support-south-florida?audience=agent-broker#proof'},
  {label:'Real Estate Investors',benefit:'Coordinate acquisitions, evictions, and REO work',to:'/projects/eviction-turnover-support?audience=investor#proof'},
  {label:'Business Owners & Operators',benefit:'Add capacity without adding another full-time hire',to:'/projects/ongoing-administrative-support?audience=business-owner#proof'},
  {label:'Attorneys & Legal Teams',benefit:'Filings, service, notary, and courthouse runs handled',to:'/projects/eviction-turnover-support?audience=attorney#proof'},
  {label:'Multi-Location Operators',benefit:'One point of contact across sites and vendors',to:'/projects/managing-business-transition?audience=operator#proof'},
]
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    <section className="pmv-hero-story relative min-h-[calc(100vh-68px)] overflow-hidden border-b border-white/[.07]"><AmbientGlow/><div className="pmv-home-grid" aria-hidden="true"/><div className="container-pmv relative z-10 grid min-h-[calc(100vh-68px)] gap-8 py-14 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-20">
      <StaggerOnMount>
        ...
        <motion.div variants={staggerItem} className="mt-9 flex flex-wrap gap-3"><Link to={CLIENT_SIGNUP} className={btnPrimary}>Tell Us What You Need</Link><Link to="/instant-quote" className={btnOutline}>Instant Cleaning / Inspection Estimate</Link><Link to="/services" className={btnOutline}>View All Services</Link></motion.div>
    ...
    <section className="border-b border-white/[.07] bg-navy-900/30"><div className="container-pmv py-10 sm:py-14"><Reveal className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-gold">Built For</p><h2 className="mt-2 font-display text-2xl font-bold tracking-[-.02em] text-white sm:text-3xl">Find your world, jump in where it fits.</h2></div><p className="max-w-md text-xs leading-6 text-slate-500">Every path lands on the same intake. Pick the one closest to your work.</p></Reveal><div className="mt-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-3" aria-label="Find support by client type">{personas.map(item=><Link key={item.label} to={item.to} className="group flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[.02] px-4 py-3 transition hover:border-gold/45 hover:bg-white/[.045]"><span className="min-w-0"><span className="block truncate text-sm font-bold text-white group-hover:text-gold">{item.label}</span><span className="mt-0.5 block truncate text-[11px] leading-5 text-slate-400">{item.benefit}</span></span><span className="text-gold transition-transform group-hover:translate-x-0.5">→</span></Link>)}</div></div></section>
