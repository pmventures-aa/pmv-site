...
import { AmbientGlow, Reveal, StaggerGroup, staggerItem } from '../components/public/motion'
...
export default function Home(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    <section className="pmv-hero-story relative min-h-[calc(100vh-68px)] overflow-hidden border-b border-white/[.07]"><AmbientGlow/><div className="pmv-home-grid" aria-hidden="true"/><div className="container-pmv relative z-10 grid min-h-[calc(100vh-68px)] gap-8 py-14 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-20">
      <Reveal><p className="eyebrow">Business · Property · Documents · Field</p><h1 className="mt-6 max-w-4xl font-display text-5xl font-extrabold leading-[.98] tracking-[-.055em] text-white sm:text-6xl lg:text-7xl xl:text-[5rem]">One relationship.<br/><span className="pmv-gold-text">No dropped handoffs.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-slate-300 sm:text-xl">Pinnacle gives business owners, property owners, and professionals one dependable place to turn when the work crosses systems, vendors, paperwork, people, and locations.</p><p className="mt-4 max-w-xl text-base leading-7 text-slate-500">Start with one request. Bring us into the bigger picture when you are ready.</p><div className="mt-9 flex flex-wrap gap-3"><a href={CLIENT_SIGNUP} className={btnPrimary}>Bring Us a Request</a><Link to="/services" className={btnOutline}>See What We Handle</Link></div><div className="mt-9 grid max-w-2xl gap-3 border-t border-white/10 pt-5 text-[11px] font-semibold uppercase tracking-[.13em] text-slate-500 sm:grid-cols-3"><span>Nationwide coordination</span><span>South Florida field team</span><span>Secure client workspace</span></div></Reveal>
      <Reveal className="relative flex min-h-[360px] items-center justify-center lg:min-h-[520px] lg:justify-end"><motion.div className="pmv-free-floating-mark" animate={{y:[0,-16,3,0],x:[0,8,-5,0],rotate:[-1,1.25,-.5,-1]}} transition={{duration:11,repeat:Infinity,ease:'easeInOut'}}><WhiteGoldBrandMark size={370} decorative/></motion.div><motion.div className="absolute bottom-5 right-0 max-w-[270px] border-l border-gold/45 pl-4 lg:bottom-14" animate={{y:[0,-5,0]}} transition={{duration:7.5,repeat:Infinity,ease:'easeInOut'}}><p className="font-display text-sm font-semibold leading-6 text-white">From first call to closeout.</p><p className="mt-1 text-xs leading-5 text-slate-500">The context stays with the work.</p></motion.div></Reveal>
    </div></section>
