...
import { AmbientGlow, ClipLine, Reveal, StaggerGroup, StaggerOnMount, staggerItem } from '../components/public/motion'
...
export default function Home(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    <section className="pmv-hero-story relative min-h-[calc(100vh-68px)] overflow-hidden border-b border-white/[.07]"><AmbientGlow/><div className="container-pmv relative z-10 grid min-h-[calc(100vh-68px)] gap-10 py-14 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-20">
      <StaggerOnMount>
        <motion.p variants={staggerItem} className="eyebrow">Nationwide professional support · South Florida field services</motion.p>
        <h1 className="mt-6 max-w-4xl font-display text-5xl font-semibold leading-[1.06] tracking-[-.03em] text-white sm:text-6xl lg:text-[3.55rem]">
          <ClipLine delay={0.06}>Business support.</ClipLine>
          <ClipLine delay={0.14}>Property services.</ClipLine>
          <ClipLine delay={0.22} className="pmv-gold-text">Documents &amp; field work.</ClipLine>
        </h1>
        ...
        <motion.div variants={staggerItem} className="mt-9 grid max-w-2xl gap-3 border-t border-white/10 pt-5 text-xs leading-5 text-slate-500 sm:grid-cols-3"><span>One-time or ongoing help</span><span>Nationwide coordination</span><span>Direct South Florida coverage</span></motion.div>
      </StaggerOnMount>
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.55,delay:.22}} className="relative lg:min-h-[480px]"><HeroGuideRail/></motion.div>
    </div></section>

    <section className="border-b border-white/[.07]"><div className="container-pmv py-12 sm:py-16"><Reveal className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Built for</p><h2 className="mt-2 font-display text-2xl font-semibold tracking-[-.02em] text-white sm:text-3xl">Find your world, jump in where it fits.</h2></div><p className="max-w-md text-sm leading-6 text-slate-500">Each path opens a dedicated operating world: property, documents, or business. The intake, portal, and follow-through match that work.</p></Reveal><div className="mt-8 divide-y divide-white/10 border-y border-white/10" aria-label="Find support by client type">{personas.map(item=><Link key={item.label} to={item.to} className="group grid gap-1 py-4 sm:grid-cols-[minmax(0,1.1fr)_minmax(0,1.4fr)_auto] sm:items-baseline sm:gap-6"><span className="text-sm font-semibold text-white group-hover:text-gold">{item.label}</span><span className="text-sm leading-6 text-slate-400">{item.benefit}</span><span className="text-xs font-semibold text-gold sm:text-right">Open</span></Link>)}</div></div></section>

    <section className="container-pmv py-16 sm:py-24"><Reveal className="grid gap-7 lg:grid-cols-[.72fr_1.28fr] lg:gap-16"><div><p className="eyebrow">Services</p><h2 className="mt-4 font-display text-3xl font-semibold leading-tight tracking-[-.03em] text-white sm:text-5xl">Professional support for the work on your list.</h2></div><p className="max-w-3xl text-lg leading-8 text-slate-400 lg:pt-7">You do not need to turn every need into a consulting engagement or manage a different company for every task. Tell Pinnacle what needs to be handled, where it is, and when you need it. We will confirm the scope and coordinate the right next step.</p></Reveal><StaggerGroup className="mt-12 grid border-y border-white/10 lg:grid-cols-3">{audiences.map((item,i)=><motion.article variants={staggerItem} key={item.title} className="group border-b border-white/10 py-10 lg:border-b-0 lg:border-r lg:px-8 lg:first:pl-0 lg:last:border-r-0 lg:last:pr-0"><div className="flex items-center justify-between"><p className="eyebrow">{item.label}</p><span className="font-display text-xs text-gold/55">{String(i+1).padStart(2,'0')}</span></div><Icon name={item.icon} size={22} className="mt-8 text-gold"/><h3 className="mt-6 font-display text-2xl font-semibold leading-[1.12] tracking-[-.02em] text-white sm:text-[1.85rem]">{item.title}</h3><p className="mt-5 text-sm leading-7 text-slate-400">{item.body}</p><div className="mt-7 space-y-2">{item.links.map(([label,to])=><Link key={to} to={to} className="flex items-center justify-between border-t border-white/[.07] pt-2 text-xs font-semibold text-slate-300 transition group-hover:text-white"><span>{label}</span><span className="text-gold">View</span></Link>)}</div></motion.article>)}</StaggerGroup></section>

    <section className="border-y border-gold/15 bg-gradient-to-br from-gold/[.04] via-transparent to-transparent"><div className="container-pmv grid gap-12 py-16 sm:py-20 lg:grid-cols-[.8fr_1.2fr] lg:gap-16"><Reveal><p className="eyebrow">How requests are handled</p><h2 className="mt-4 font-display text-3xl font-semibold leading-tight tracking-[-.03em] text-white sm:text-5xl">A clear process from request to completion.</h2><p className="mt-5 max-w-lg text-base leading-8 text-slate-400">Pinnacle gives you one place to explain the need, receive updates, share documents, and confirm the work is complete - even when several providers or steps are involved.</p><Link to="/about" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-gold">How Pinnacle works <span>→</span></Link></Reveal><div className="divide-y divide-white/10 border-y border-white/10">{difference.map(([title,body],i)=><Reveal key={title} delay={i*.04} className="grid gap-3 py-6 sm:grid-cols-[44px_170px_1fr] sm:items-start"><span className="font-display text-xs font-semibold text-gold/60">{String(i+1).padStart(2,'0')}</span><h3 className="font-display text-sm font-semibold text-white">{title}</h3><p className="text-sm leading-6 text-slate-400">{body}</p></Reveal>)}</div></div></section>
