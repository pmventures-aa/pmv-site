...
const personas=[
  ['Landlord','/projects/property-cleaning-turnover?audience=landlord#proof'],
  ['Agent · Broker','/projects/local-support-south-florida?audience=agent-broker#proof'],
  ['Investor','/projects/eviction-turnover-support?audience=investor#proof'],
  ['Business Owner','/projects/ongoing-administrative-support?audience=business-owner#proof'],
  ['Operator','/projects/managing-business-transition?audience=operator#proof'],
  ['Attorney','/projects/local-support-south-florida?audience=attorney#proof'],
  ['Corporate','/projects/switching-pos-payment-providers?audience=corporate#proof'],
]
...
export default function Home(){
  ...
  return <div className="min-h-screen bg-navy-950"><Header/><main>
    <section className="pmv-hero-story relative min-h-[calc(100vh-68px)] overflow-hidden border-b border-white/[.07]"><AmbientGlow/><div className="pmv-home-grid" aria-hidden="true"/><div className="container-pmv relative z-10 grid min-h-[calc(100vh-68px)] gap-8 py-14 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-20">
      <StaggerOnMount>
        <motion.div variants={staggerItem} className="mb-7 flex flex-wrap gap-2" aria-label="Find support by client type">{personas.map(([label,to])=><Link key={label} to={to} className="rounded-full border border-white/10 bg-white/[.025] px-3 py-1.5 text-[11px] font-bold text-slate-300 transition hover:border-gold/40 hover:text-gold">I’m a {label}</Link>)}</motion.div>
    ...
    <section className="container-pmv py-16 sm:py-24"><div className="grid gap-10 lg:grid-cols-[.58fr_1.42fr] lg:items-start lg:gap-14"><Reveal><p className="eyebrow">Start a request</p><h2 className="mt-4 font-display text-3xl font-bold tracking-[-.035em] text-white sm:text-4xl">Simple tasks are welcome.</h2><p className="mt-5 max-w-md text-sm leading-7 text-slate-400">A cleaning, inspection, document, notary appointment, field visit, administrative task, or larger project can all begin here. No account and no formal project brief required.</p><p className="mt-4 text-sm font-semibold text-white">A person replies within two business hours.</p></Reveal><ScopeWizard source="home" compact/></div></section>
