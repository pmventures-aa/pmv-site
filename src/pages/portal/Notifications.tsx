...
export default function Notifications() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Updates" title="Notifications" subtitle="Choose how Pinnacle keeps you informed, then see recent account activity below." />
      {loading ? <p className="text-sm text-slate-400">Loading…</p> : <>
        <Card className="mb-6">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-base font-semibold text-white">How should we keep you updated?</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">Keep important service and account updates in the portal, by email, or both. Security and required account notices may still be delivered when necessary.</p></div><button disabled={saving} onClick={save} className="btn-gold">{saving?'Saving…':'Save preferences'}</button></div>
