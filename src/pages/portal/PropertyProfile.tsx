      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
        <form onSubmit={onSave} className="space-y-3">
          ...
          <Card>
            ...
            <button type="submit" disabled={busy} className="btn-gold mt-3 disabled:opacity-60">{busy ? 'Saving…' : 'Save profile'}</button>
          </Card>
        </form>
        ...
        <aside className="space-y-3">
          ...
          <Card className="!p-0">
            ...
            {tickets.length === 0 ? <div className="p-3"><EmptyState label="No requests tied to this address." /></div> : (
              <ul className="divide-y divide-white/10">
                {tickets.map((t) => (
                  <li key={t.id}><Link to={p('support')} className="block px-3 py-2 hover:bg-white/[.03]"><p className="text-sm font-medium text-white">{t.subject}</p><p className="mt-0.5 text-xs text-slate-500">{t.status.replace(/_/g, ' ')}</p></Link></li>
                ))}
              </ul>
            )}
          </Card>
          <Card className="!p-0">
            ...
            {documents.length === 0 ? <div className="p-3"><EmptyState label="No files attached to this address." /></div> : (
              <ul className="divide-y divide-white/10">
                {documents.map((d) => (
