...
export function describeActivity(e: ActivityEvent): string {
  ...
  switch (e.kind) {
    ...
    case 'auth0_linked': return `${actor} connected ${d.provider || 'an Auth0'} sign-in`
    case 'auth0_unlinked': return `${actor} disconnected an Auth0 sign-in method`
    case 'auth0_link_failed': return `${actor} could not connect an Auth0 sign-in method`
