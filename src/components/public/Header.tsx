...
const serviceLinks=[
  {to:'/services/property-field',label:'Property & Field',body:'Inspections, cleaning, turnovers, REO, eviction support, access, and on-site follow-through.'},
  {to:'/services/mobile-documents',label:'Documents & Signing',body:'Preparation, filing, courier, mobile notary, RON, and coordinated signing support.'},
  {to:'/services/business-operations',label:'Business & Operations',body:'Admin capacity, systems, POS and payments, process work, and project coordination.'},
]
...
export function Header(){
  ...
  >
    <div className="container-pmv flex h-[68px] items-center justify-between gap-4">
      <motion.div animate={{scale:scrolled ? .96 : 1}} transition={pmvMotion.ui} style={{transformOrigin:'left center'}}>
        <Link to="/" aria-label="Pinnacle Management Ventures home" className="block shrink-0 transition-opacity hover:opacity-90"><Logo/></Link>
      </motion.div>
      ...
      <div className="hidden items-center gap-2 xl:flex"><a href={CLIENT_LOGIN} className={btnOutline}>Client Sign In</a><a href={CLIENT_SIGNUP} className={btnPrimary}>Start a Request</a></div>
