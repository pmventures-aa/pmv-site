...
export function PageIntro({ kicker, title, subtitle, align='left' }: { kicker:string; title:string; subtitle?:string; align?:'left'|'center' }) { const centered=align==='center';return <div className={`${centered?'mx-auto max-w-4xl text-center':'max-w-4xl'}`}><Reveal><p className="eyebrow">{kicker}</p><h1 className="pmv-h1 mt-4">{title}</h1>{subtitle&&<p className={`mt-5 max-w-2xl text-lg leading-8 text-slate-300 ${centered?'mx-auto':''}`}>{subtitle}</p>}</Reveal></div> }
