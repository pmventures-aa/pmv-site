...
export const btnPrimary = 'inline-flex items-center justify-center gap-2 rounded-md bg-gold px-6 py-3 text-sm font-semibold text-navy-950 transition-colors duration-200 hover:bg-gold-400'
export const btnOutline = 'inline-flex items-center justify-center gap-2 rounded-md border border-white/18 px-6 py-3 text-sm font-semibold text-slate-100 transition-colors duration-200 hover:border-gold/55 hover:text-gold'
...
export function TagLine({ tag, popular }: { tag: string; popular?: boolean }) {
  return <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-gold/85">{tag}{popular ? ' · Featured' : ''}</p>
}

export function MotionStage() {
  return <div className="pmv-motion-stage" aria-hidden="true"><div className="pmv-orbit pmv-orbit-one"/><div className="pmv-orbit pmv-orbit-two"/><motion.div className="pmv-stage-card pmv-stage-card-back" animate={{y:[0,-8,0],rotateZ:[-5,-3,-5]}} transition={{duration:8,repeat:Infinity,ease:'easeInOut'}}><span>Property</span><strong>Field work</strong><small>Visits · Vendors · Documents</small></motion.div><motion.div className="pmv-stage-card pmv-stage-card-main" animate={{y:[0,7,0],rotateY:[0,3,0]}} transition={{duration:7,repeat:Infinity,ease:'easeInOut'}}><div className="pmv-stage-topline"><span>Pinnacle workspace</span><i/></div><div className="pmv-stage-bars"><b/><b/><b/></div><div className="pmv-stage-grid"><span/><span/><span/><span/></div><div className="pmv-stage-footer"><span>Projects</span><span>Messages</span><span>Documents</span></div></motion.div><motion.div className="pmv-stage-card pmv-stage-card-front" animate={{y:[0,-10,0],rotateZ:[5,3,5]}} transition={{duration:6.5,repeat:Infinity,ease:'easeInOut'}}><span>Operations</span><strong>Next step ready</strong><small>Consulting · Systems · Support</small></motion.div><motion.div className="pmv-stage-pill pmv-stage-pill-one" animate={{y:[0,-12,0],x:[0,4,0]}} transition={{duration:5.5,repeat:Infinity,ease:'easeInOut'}}>Client update</motion.div><motion.div className="pmv-stage-pill pmv-stage-pill-two" animate={{y:[0,9,0],x:[0,-4,0]}} transition={{duration:6.2,repeat:Infinity,ease:'easeInOut'}}>Assigned</motion.div></div>
}
...
export function PageIntro({ kicker, title, subtitle, align='left' }: { kicker:string; title:string; subtitle?:string; align?:'left'|'center' }) {
  ...
  return <div className={`${centered?'mx-auto max-w-4xl text-center':'max-w-4xl'}`}><Reveal><p className="eyebrow">{kicker}</p><h1 className="mt-4 font-display text-4xl font-medium tracking-[-.025em] text-white sm:text-5xl lg:text-[3.5rem]">{title}</h1>{subtitle&&<p className={`mt-5 max-w-2xl text-lg leading-8 text-slate-300 ${centered?'mx-auto':''}`}>{subtitle}</p>}</Reveal></div>
}
...
export function CtaBand({ title, body, primary, secondary }: { title:string; body:string; primary:{to:string;label:string}; secondary?:{to:string;label:string} }) {
  return <section className="border-y border-white/10 bg-navy-900/55"><Reveal className="container-pmv flex flex-col items-start gap-8 py-14 sm:py-16 lg:flex-row lg:items-center lg:justify-between"><div className="max-w-2xl"><p className="eyebrow">Next step</p><h2 className="mt-3 font-display text-3xl font-medium tracking-tight text-white sm:text-4xl">{title}</h2><p className="mt-4 max-w-xl text-base leading-7 text-slate-300">{body}</p></div><div className="flex flex-wrap gap-3"><Link to={primary.to} className={btnPrimary}>{primary.label}</Link>{secondary&&<Link to={secondary.to} className={btnOutline}>{secondary.label}</Link>}</div></Reveal></section>
}
