...
export function RichTextComposer({
  ...
  const editorClass = letter
    ? `${letterFlow ? 'min-h-[240px]' : fill ? 'h-full min-h-[240px] overflow-y-auto' : 'min-h-48'} px-4 py-5 font-serif text-[15px] leading-[1.75] text-[#1b2430] outline-none sm:px-8 [&_a]:text-[#0a1728] [&_a]:underline [&_img]:my-2 [&_img]:max-w-full [&_[align=center]]:text-center [&_[align=right]]:text-right [&_[align=left]]:text-left`
  ...
  return (
    <div className={`flex min-h-0 flex-col ${fill ? 'h-full overflow-hidden' : ''} ${letter ? 'bg-transparent' : 'rounded-md border border-white/10 bg-navy-900'}`}>
      <div className={`flex shrink-0 flex-nowrap items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:flex-wrap sm:overflow-visible ${letter ? 'border-b border-[#e7e4dc] bg-[#f4f1ea] px-3 py-1.5' : 'border-b border-white/10 p-2 gap-1.5'}`}>
        {letter && (
          <>
            <select
              className="h-8 max-w-[7.5rem] shrink-0 rounded border border-[#ddd6c8] bg-white px-1 text-[12px] text-[#1b2430]"
              defaultValue="P"
            ...
            <select
              className="h-8 max-w-[8.5rem] shrink-0 rounded border border-[#ddd6c8] bg-white px-1 text-[12px] text-[#1b2430]"
            ...
            <select
              className="h-8 w-[4.4rem] shrink-0 rounded border border-[#ddd6c8] bg-white px-1 text-[12px] text-[#1b2430]"
              defaultValue="15px"
              title="Size"
              onMouseDown={saveSelection}
      ...
      <div className={`relative min-h-0 ${fill ? (letterFlow ? 'flex-1 overflow-y-auto' : 'flex-1 overflow-hidden') : ''}`}>
        <div className={`relative ${fill && !letterFlow ? 'h-full' : ''}`}>
          {empty && placeholder && (
            <p className={`pointer-events-none absolute ${letter ? 'left-4 top-5 font-serif text-[15px] italic text-[#9aa3ae] sm:left-8' : 'left-4 top-3 text-sm text-slate-500'}`}>{placeholder}</p>
          )}
          <div
            ref={ref}
            contentEditable
