...
export async function renderOfficialAuditCertificate(env:Env,envelope:any,recipients:any[],events:any[],signedPdfHash:string,chainHead:string,sealedAt:string){
  ...
  page.drawCircle({x:62,y:475,size:9,color:green})
  page.drawLine({start:{x:57.5,y:475},end:{x:61,y:471.5},thickness:1.5,color:rgb(1,1,1)})
  page.drawLine({start:{x:61,y:471.5},end:{x:67,y:479},thickness:1.5,color:rgb(1,1,1)})
  ...
  let y=414
  for(const [l,v] of pairs){page.drawText(l.toUpperCase(),{x:42,y,size:6.4,font:bold,color:muted});page.drawText(v,{x:190,y:y-1,size:9.2,font:l==='Envelope / Transaction ID'?bold:regular,color:ink,maxWidth:370});page.drawLine({start:{x:42,y:y-11},end:{x:570,y:y-11},thickness:.45,color:rgb(.87,.88,.89)});y-=36}
  ...
  const legalLines=wrap(legal,116).slice(0,5);let ly=82;for(const l of legalLines){page.drawText(l,{x:42,y:ly,size:6.3,font:regular,color:muted,maxWidth:528});ly-=9}
