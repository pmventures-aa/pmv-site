...
async function requireSignerSession(c: any, recipient: any) {
  const raw = c.req.header('x-signing-session') || c.req.query('session') || ''
  if (!raw) return false
  const stored = await c.env.SESSIONS.get(`sign:${raw}`)
  if (!stored) return false
  try {
    const ses = JSON.parse(stored)
    return ses?.recipient_id === recipient.id && ses?.envelope_id === recipient.envelope_id
  } catch {
    return false
  }
}
...
documentSignerFileRoutes.get('/sign/:token/documents/:documentId/file', async c => {
  ...
  if (!(await requireSignerSession(c, r))) return c.json({ error: 'authentication required' }, 401)
  ...
  return new Response(obj.body, {
    headers: {
      'Content-Type': f.mime_type || 'application/pdf',
      'Content-Disposition': `inline; filename="${String(f.original_name || 'document.pdf').replace(/"/g, '')}"`,
