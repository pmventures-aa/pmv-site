...
documentSignerFileRoutes.get('/sign/:token/documents/:documentId/file',async c=>{
  ...
  const f=await c.env.DB.prepare('SELECT storage_key,mime_type,original_name FROM document_files WHERE id=?').bind(fileId).first() as any;if(!f)return c.json({error:'file not found'},404)
