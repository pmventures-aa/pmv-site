...
resendWebhookRoutes.post('/webhooks/resend', async (c) => {
  ...
  // Signature verification must use the exact raw body. Parsing and then
  // re-stringifying JSON changes bytes and invalidates Svix/Resend signatures.
  ...
  // Resend/Svix retries non-2xx deliveries. Store the signed message ID before
  // mutating email state so a retry becomes a harmless 200 duplicate.
