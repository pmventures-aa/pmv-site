...
messageRoutes.get('/message-threads/:id', async (c) => {
  ...
  const [messages, attachments, reads] = await Promise.all([
    ...
    c.env.DB.prepare(
      `SELECT tr.user_id,tr.last_read_at,u.role,u.full_name,u.email
       FROM thread_reads tr JOIN users u ON u.id=tr.user_id WHERE tr.thread_id=?`,
    ).bind(thread.id).all<any>(),
  ])

  await markRead(c.env, thread.id, user.id).run()
  const readRows = reads.results ?? []
  const enriched = (messages.results ?? []).map((m:any) => {
    const otherReads = readRows.filter((r:any) => r.user_id !== m.sender_user_id)
    const readBy = otherReads.filter((r:any) => new Date(r.last_read_at).getTime() >= new Date(m.created_at).getTime())
    return { ...m, delivery_status: 'delivered', read_at: readBy.length ? readBy.sort((a:any,b:any) => new Date(a.last_read_at).getTime()-new Date(b.last_read_at).getTime())[0].last_read_at : null }
  })
  return c.json({ thread, messages: enriched, attachments: attachments.results ?? [] })
})
...
messageRoutes.post('/message-threads', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`INSERT INTO message_threads (id, client_user_id, subject, created_by) VALUES (?, ?, ?, ?)`)
...
async function notifyNewMessage(env: Env, sender: SessionUser, clientId: string, subject: string, body: string): Promise<void> {
  ...
  await notifyStaff(env, {
    ...
    html: `<p><strong>${escapeHtml(senderLabel)}</strong> sent a secure message${sender.role === 'client' ? '' : ` to ${escapeHtml(clientLabel)}`} in “${escapeHtml(subject)}”:</p><p>${escapeHtml(body)}</p>`,
  })
}
...
messageRoutes.post('/message-threads/:threadId/messages/:messageId/attachments', async (c) => {
  ...
  if (bodyBuf.byteLength === 0) return c.json({ error: 'The uploaded file is empty.' }, 400)
  if (bodyBuf.byteLength > MAX_ATTACHMENT_BYTES) return c.json({ error: 'File exceeds the 15 MB attachment limit.' }, 413)

  const attachmentId = uuid()
  const key = `messages/${thread.id}/${attachmentId}.${ext}`
  await c.env.UPLOADS.put(key, bodyBuf, { httpMetadata: { contentType } })
  await c.env.DB.batch([
    c.env.DB.prepare(`INSERT INTO message_attachments (id, message_id, r2_key, file_name, content_type, size_bytes) VALUES (?, ?, ?, ?, ?, ?)`)
