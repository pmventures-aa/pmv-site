...
roleCapabilityRoutes.get('/effective-capabilities', requireStaff, async (c) => {
  ...
  const [banking, users, settings, reports, audit, communications, invitations, documents, team] = await Promise.all([
    ...
    hasNamedPermission(c.env,user,'manage_invitations'),
    hasNamedPermission(c.env,user,'manage_documents'),
    hasNamedPermission(c.env,user,'manage_team'),
  ])
  return c.json({
    ...
    can_manage_invitations: invitations,
