...
reportRoutes.post('/report-templates/:id/run',requireStaff,requireCapability('can_view_reports'),async(c)=>{
  const user=c.get('user');const template=await c.env.DB.prepare('SELECT * FROM report_templates WHERE id=? AND created_by=?').bind(c.req.param('id'),user.id).first<any>();if(!template)return c.json({error:'not found'},404);const def=REPORTS_BY_KEY[template.report_key];if(!def)return c.json({error:'unknown report'},400);if(!canRunReport(user,def))return c.json({error:'forbidden'},403);let filters:any={};try{const parsed=JSON.parse(template.filters_json||'{}');filters=parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:{}}catch{filters={}}const{from,to}=dateRange({req:{query:(k:string)=>filters[k]}});const ctx:ReportCtx={env:c.env,from,to,clientIds:await visibleClientIds(c.env,user)};const runId=uuid()
