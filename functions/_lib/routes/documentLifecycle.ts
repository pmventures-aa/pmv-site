...
documentLifecycleAdminRoutes.get('/envelopes',async c=>{ const rows=await c.env.DB.prepare(`SELECT e.*, (SELECT COUNT(*) FROM envelope_recipients r WHERE r.envelope_id=e.id) signer_count, (SELECT COUNT(*) FROM envelope_recipients r WHERE r.envelope_id=e.id AND r.status='completed') completed_signer_count FROM envelopes e ORDER BY e.created_at DESC LIMIT 200`).all(); return c.json({envelopes:rows.results}) })
...
documentLifecycleAdminRoutes.post('/documents/upload',async c=>{
  ...
  let name=c.req.header('X-File-Name')||'document.pdf', mime=c.req.header('content-type')||'application/octet-stream', bytes:ArrayBuffer
  if(mime.includes('multipart/form-data')){ const form=await c.req.formData(); const file=form.get('file') as File|null; if(!file)return c.json({error:'file is required'},400); name=file.name; mime=file.type||'application/octet-stream'; bytes=await file.arrayBuffer() }
  else bytes=await c.req.arrayBuffer()
  if(!bytes.byteLength)return c.json({error:'file is required'},400)
  if(mime!=='application/pdf'&&!name.toLowerCase().endsWith('.pdf')) return c.json({error:'PDF source documents are required for signing and final rendering.'},400)
  const hash=await sha256Hex(bytes), id=uuid(), safe=name.replace(/[^a-zA-Z0-9._-]/g,'_'), key=`documents/source/${id}/${safe}`
  await c.env.UPLOADS.put(key,bytes,{httpMetadata:{contentType:mime},customMetadata:{sha256:hash,kind:'source'}})
  await c.env.DB.prepare(`INSERT INTO document_files (id,kind,original_name,mime_type,storage_key,size_bytes,sha256,created_by_user_id) VALUES (?,?,?,?,?,?,?,?)`).bind(id,'source',name,mime,key,bytes.byteLength,hash,c.get('user').id).run()
  return c.json({file:{id,name,mime_type:mime,size_bytes:bytes.byteLength,sha256:hash}},201)
})

documentLifecycleAdminRoutes.post('/envelopes',async c=>{ const b=await c.req.json<any>().catch(()=>null); if(!b?.title||!b?.source_file_id)return c.json({error:'title and source_file_id are required'},400); const id=uuid(), publicId=`PMV-${new Date().getUTCFullYear()}-${uuid().split('-')[0].toUpperCase()}`; await c.env.DB.prepare(`INSERT INTO envelopes (id,public_id,title,message,client_id,source_file_id,canonical_file_id,status,signing_order_mode,expires_at,created_by_user_id) VALUES (?,?,?,?,?,?,?,'draft',?,?,?)`).bind(id,publicId,String(b.title).slice(0,200),b.message||null,b.client_id||null,b.source_file_id,b.source_file_id,b.signing_order_mode==='sequential'?'sequential':'parallel',b.expires_at||null,c.get('user').id).run(); await appendEvent(c,id,'envelope.created','staff',c.get('user').id,null,{title:b.title}); return c.json({id,public_id:publicId},201) })

documentLifecycleAdminRoutes.get('/envelopes/:id',async c=>{ const id=c.req.param('id'), envelope=await c.env.DB.prepare('SELECT * FROM envelopes WHERE id=?').bind(id).first(); if(!envelope)return c.json({error:'not found'},404); const [recipients,fields,events,seal]=await Promise.all([c.env.DB.prepare('SELECT * FROM envelope_recipients WHERE envelope_id=? ORDER BY routing_order,created_at').bind(id).all(),c.env.DB.prepare('SELECT * FROM document_fields WHERE envelope_id=? ORDER BY page_number,created_at').bind(id).all(),c.env.DB.prepare('SELECT * FROM envelope_events WHERE envelope_id=? ORDER BY occurred_at_utc,id').bind(id).all(),c.env.DB.prepare('SELECT * FROM envelope_seals WHERE envelope_id=?').bind(id).first()]); return c.json({envelope,recipients:recipients.results,fields:fields.results,events:events.results,seal}) })

documentLifecycleAdminRoutes.post('/envelopes/:id/recipients',async c=>{ const envId=c.req.param('id'), b=await c.req.json<any>().catch(()=>null); if(!b?.name||!b?.email)return c.json({error:'name and email required'},400); const id=uuid(); await c.env.DB.prepare(`INSERT INTO envelope_recipients (id,envelope_id,role,routing_order,name,email,phone_e164,status) VALUES (?,?,?,?,?,?,?,'pending')`).bind(id,envId,b.role||'signer',Number(b.routing_order||1),String(b.name).slice(0,160),String(b.email).toLowerCase().slice(0,200),b.phone_e164||null).run(); await appendEvent(c,envId,'recipient.added','staff',c.get('user').id,id,{email:b.email,role:b.role||'signer'}); return c.json({id},201) })

documentLifecycleAdminRoutes.post('/envelopes/:id/fields',async c=>{ const envId=c.req.param('id'), b=await c.req.json<any>().catch(()=>null), allowed=['signature','initial','date','text','checkbox']; if(!b?.recipient_id||!allowed.includes(b.field_type))return c.json({error:'invalid field'},400); for(const k of ['x','y','width','height'])if(typeof b[k]!=='number'||b[k]<0||b[k]>1)return c.json({error:`invalid ${k}`},400); const id=uuid(); await c.env.DB.prepare(`INSERT INTO document_fields (id,envelope_id,recipient_id,page_number,field_type,x,y,width,height,required,label,validation_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id,envId,b.recipient_id,Number(b.page_number||1),b.field_type,b.x,b.y,b.width,b.height,b.required===false?0:1,b.label||null,b.validation?JSON.stringify(b.validation):null).run(); await appendEvent(c,envId,'field.placed','staff',c.get('user').id,b.recipient_id,{field_type:b.field_type,page_number:b.page_number||1,x:b.x,y:b.y}); return c.json({id},201) })

documentLifecycleAdminRoutes.post('/envelopes/:id/send',async c=>{ const envId=c.req.param('id'), env=await c.env.DB.prepare('SELECT * FROM envelopes WHERE id=?').bind(envId).first<any>(); if(!env)return c.json({error:'not found'},404); const rs=await c.env.DB.prepare(`SELECT * FROM envelope_recipients WHERE envelope_id=? AND role IN ('signer','approver','witness','notary') ORDER BY routing_order`).bind(envId).all<any>(); if(!rs.results.length)return c.json({error:'add at least one signer'},400); await c.env.DB.prepare(`UPDATE envelopes SET status='sent',sent_at=?,updated_at=? WHERE id=?`).bind(now(),now(),envId).run(); for(const r of rs.results){ const raw=token(),hash=await sha256Hex(raw),exp=new Date(Date.now()+7*86400000).toISOString(); await c.env.DB.prepare(`UPDATE envelope_recipients SET status='invited',invitation_token_hash=?,invitation_expires_at=?,updated_at=? WHERE id=?`).bind(hash,exp,now(),r.id).run(); const url=`https://www.pinnaclemanagementventures.com/sign/${encodeURIComponent(raw)}`; c.executionCtx.waitUntil(sendEmail(c.env,{to:r.email,subject:`Signature requested: ${env.title}`,html:`<p>${escapeHtml(r.name)},</p><p>Pinnacle Management Ventures has sent you a document for review and signature.</p><p><a href="${url}">Review and sign document</a></p><p>This secure link expires in 7 days.</p>`}).catch(()=>{})) } await appendEvent(c,envId,'envelope.sent','staff',c.get('user').id,null,{recipient_count:rs.results.length}); return c.json({ok:true}) })

documentLifecycleAdminRoutes.post('/envelopes/:id/seal',async c=>{ const id=c.req.param('id'); const env=await c.env.DB.prepare('SELECT status FROM envelopes WHERE id=?').bind(id).first<any>(); if(!env)return c.json({error:'not found'},404); if(env.status!=='completed')return c.json({error:'Only completed envelopes can be sealed.'},400); const existing=await c.env.DB.prepare('SELECT * FROM envelope_seals WHERE envelope_id=?').bind(id).first<any>(); if(existing)return c.json({ok:true,seal:existing,already_sealed:true}); const result=await sealEnvelope(c.env,id); await appendEvent(c,id,'envelope.sealed','system',null,null,{algorithm:result.algorithm,key_id:result.keyId,final_pdf_sha256:result.signedPdfHash,audit_pdf_sha256:result.auditPdfHash,manifest_sha256:result.manifestHash}); const signerCount=await c.env.DB.prepare(`SELECT COUNT(*) n FROM envelope_recipients WHERE envelope_id=? AND role='signer'`).bind(id).first<any>(); const e=await c.env.DB.prepare('SELECT public_id,title,completed_at FROM envelopes WHERE id=?').bind(id).first<any>(); await c.env.DB.prepare(`INSERT OR REPLACE INTO verification_records (id,envelope_id,public_id,verification_status,document_label,issuer_name,completed_date,signer_count,final_pdf_sha256_prefix,sealed_at,updated_at) VALUES (?,?,?,?,?,'Pinnacle Management Ventures',?,?,?,?,?)`).bind(uuid(),id,e.public_id,'valid',e.title,e.completed_at||result.sealedAt,Number(signerCount?.n||0),result.signedPdfHash.slice(0,16),result.sealedAt,now()).run(); return c.json({ok:true,seal:result}) })

documentLifecycleAdminRoutes.get('/envelopes/:id/artifacts/:kind',async c=>{ if(!c.env.UPLOADS)return c.json({error:'Document storage is not configured.'},503); const col:{[k:string]:string}={signed:'final_signed_file_id',audit:'audit_certificate_file_id',manifest:'evidence_manifest_file_id'}; const column=col[c.req.param('kind')]; if(!column)return c.json({error:'unknown artifact'},400); const row=await c.env.DB.prepare(`SELECT f.storage_key,f.mime_type,f.original_name FROM envelopes e JOIN document_files f ON f.id=e.${column} WHERE e.id=?`).bind(c.req.param('id')).first<any>(); if(!row)return c.json({error:'artifact not found'},404); const obj=await c.env.UPLOADS.get(row.storage_key); if(!obj)return c.json({error:'artifact missing from storage'},404); return new Response(obj.body,{headers:{'Content-Type':row.mime_type||'application/octet-stream','Content-Disposition':`inline; filename="${String(row.original_name||'document').replace(/"/g,'')}"`,'Cache-Control':'private, no-store'}}) })
