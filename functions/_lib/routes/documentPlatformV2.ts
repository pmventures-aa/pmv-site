...
documentPlatformV2AdminRoutes.post('/document-platform/templates/:templateId/use',async c=>{
  ...
  await c.env.DB.prepare(`INSERT INTO envelopes(id,public_id,title,message,client_id,template_id,source_file_id,canonical_file_id,status,signing_order_mode,expires_at,created_by_user_id,priority,approval_status,reminder_frequency,tags_json,last_activity_at,branding_profile_id) VALUES (?,?,?,?,?,?,?,?, 'draft',?,?,?,?,?,?,?,?,?,?)`).bind(envId,publicId,String(body.title||t.title_template||t.name).slice(0,200),body.message??t.message_template,body.client_id||null,null,sourceFileId,settings.canonical_file_id||sourceFileId,t.signing_order_mode,expiration,c.get('user').id,t.priority,t.approval_status_default,t.reminder_frequency,t.tags_json||'[]',now(),t.branding_profile_id||null).run()
...
documentPlatformV2AdminRoutes.patch('/document-platform/envelopes/:envelopeId/fields/:fieldId/rules',async c=>{
  ...
  await c.env.DB.prepare('UPDATE document_fields SET required=?,label=?,validation_json=?,condition_json=?,validation_error_message=?,updated_at=? WHERE id=?').bind(b.required===false?0:b.required===true?1:f.required,b.label!==undefined?String(b.label).slice(0,180):f.label,JSON.stringify(validation),Object.keys(condition).length?JSON.stringify(condition):null,b.error_message!==undefined?String(b.error_message).slice(0,300):f.validation_error_message,now(),f.id).run()
